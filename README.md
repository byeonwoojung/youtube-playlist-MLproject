...
- **썸네일 분석**: 텍스트 비율, 색상 분포, 밝기 대비, 질감, 얼굴 탐지, 객체 탐지
- **오디오 분석**: 7가지 감정 분류 (Audio Emotional: `happy`, `sad`, `angry`, `fear`, `surprise`, `disgust`, `neutral`), 음악적 특성(BPM, 피치, 에너지 등)
- **제목 분석**: 이모지 여부, 특수문자, 해시태그, 문자 길이
- **시간대 분석**: 업로드 시간대와 콘텐츠 타입 매칭
...
### 1. 썸네일 피처 (Thumbnail Features)

| 피처                                             | 설명                                | 추출 방법          |
| ------------------------------------------------ | ----------------------------------- | ------------------ |
| `text_ratio`                                   | 썸네일 내 텍스트 면적 비율          | Google Vision OCR  |
| `colorsCluster_0~44`                           | CSS3 45개 색상 클러스터별 면적 비율 | K-Means 클러스터링 |
| `total_colors`                                 | 사용된 색상 클러스터 개수           | 색상 다양성        |
| `brightness_std`                               | 밝기 표준편차 (대비)                | Grayscale 분석     |
| `texture_sharpness`                            | 질감 및 선명도                      | Laplacian 변환     |
| `person`, `animal`, `anime`, `landscape` | 객체 탐지                           | YOLO/CNN           |
| `total_faces`, `frontal_faces_8_percent`     | 얼굴 탐지 및 정면 얼굴              | YuNet 모델         |
| `theme_match_score`                            | 색상 테마 일관성                    | 클러스터 분포      |
...
2. **Audio Emotional 피처** - `happy`, `sad` 등 감정 확률
